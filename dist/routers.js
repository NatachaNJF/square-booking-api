...
export const appRouter = router({
    ...
    pricing: router({
        updateRoom: protectedProcedure
            ...
            if (ctx.user.role !== 'admin')
                throw new TRPCError({ code: 'UNAUTHORIZED', message: 'Réservé aux administrateurs' });
    ...
    calendar: router({
        getReservations: publicProcedure
            ...
            .query(async ({ input }) => getReservationsForCalendar(new Date(input.startDate), new Date(input.endDate))),
    }),
    // ─── Réservations ──────────────────────────────────────────────────────────
    reservations: router({
        ...
        checkAvailability: publicProcedure
            ...
            const roomsToCheck = [input.roomId];
            if (input.roomId === 3) {
            ...
            const allConflicts = await Promise.all(roomsToCheck.map(id => checkAvailability(id, new Date(input.startTime), new Date(input.endTime))));
        ...
        create: publicProcedure
            ...
            if (!room)
                throw new TRPCError({ code: 'NOT_FOUND', message: 'Salle introuvable' });
            ...
            if (durationHours > 1 && priceHalfDay < basePrice)
                basePrice = priceHalfDay;
            if (durationHours > 4 && priceFullDay < basePrice)
                basePrice = priceFullDay;
            ...
            if (input.selectedOptions && input.selectedOptions.length > 0) {
                await Promise.all(input.selectedOptions.map(opt => addReservationOption(reservation.id, opt.optionId, opt.quantity, opt.price)));
            }
            return reservation;
        }),
